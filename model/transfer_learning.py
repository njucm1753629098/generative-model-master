...
def Transfer(restore_from = None):
    ...
    moldata = MolData("data/inhibitor2_augmented3.txt", voc)
    
    data = DataLoader(moldata, batch_size=50, shuffle=True, drop_last=False,
    ...
    for epoch in range(1, 201):
        ...
        torch.save(Prior.rnn.state_dict(), "data/200_epochs_transfer_augument3_one.ckpt")

if __name__ == "__main__":
    smiles_file = 'data/inhibitor2.txt'
    voc_file = './Voc'
    print("Reading smiles...")
    smiles_list = ds.canonicalize_smiles_from_file(smiles_file)
    ds.filter_file_on_chars(smiles_list,voc_file)
    Transfer('data/50_epoch.ckpt')
